...
- 按照[第二步](#第二步：規劃第一主修-(IDA))規劃該專業的必修課 (Required Courses)、研究項目 (Research Component) 和選修課 (Electives)。
...
- **學分 (Credits) 限制提醒**：
  ...
  - **暑修學期 (Summer Term)**：上限 6 學分 (Credits)。
  - **全學年**：18 - 39 學分 (Credits)。
  - 如果超出或低於限制，系統會彈出**紅色警告**，提醒你需要提交超修/減修申請。
...
- 在 **"Graduation Requirements"** 子標籤頁 (Sub-Tab) 中，你可以直觀地看到：
  ...
  - 哪些要求已滿足（Fulfil 列顯示為 True），哪些仍未足夠。
...
## 💡 小貼士
- **自動保存**：應用基於 Streamlit 運行，在當前會話中你的選擇會自動保留，但刷新頁面可能會導致重置。建議規劃完成後及時導出。
- **學分 (Credits) 提醒**：如果某門課的學分有變動，你可以在表格中直接修改 Credits 列。
- **官方參考**：本工具僅供參考，請查閱 CUSIS / SIS 等官方系統和文件。
